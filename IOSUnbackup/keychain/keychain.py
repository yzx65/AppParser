...
from util.asciitables import readUtf8Text
...
def render_password(p):
    ...
    try:
        if p.has_key("srvr"):
            result =  "%s:%d;%s;%s" % (p["srvr"],p["port"],p["acct"],data)
        else:
            result = "%s;%s;%s" % (p["svce"],p["acct"],data)
    except:
        pass
...
class Keychain(object):
    ...
    def print_all(self, sanitize=True):
        ...
        headers = ["Server", "Account", "Data", "Access group", "Protection class", "Description"]
        rows = []
        ...
        for p in self.get_inet_passwords():
            ...
            row = [addr,
                   #jsc
                   #str(p.get("acct","?")),
                   #p.get("acct","?").encode("GB18030"),
                   readUtf8Text(p.get("acct","?")),
                   self.sanitize(p.get("data","?"))[:20],
                   p.get("agrp","?"),
                   #jsc
                   #KSECATTRACCESSIBLE.get(p["clas"])[18:]]
                   KSECATTRACCESSIBLE.get(p["clas"])[18:],
                   #p.get("desc","?").encode("GB18030")
                   readUtf8Text(p.get("desc","?")),
                   ]              
