...
class Keybag(object):
    ...
    def createWithBackupManifest(manifest, password, deviceKey=None):
        ...
        #print globalvar.g_argvPwd
        #jsc 20141020
        if len(password) == 0:
            print time.strftime('Start at %H:%M:%S', time.localtime(time.time()))
            nFrom = string.atol(globalvar.g_argvTryPwdFrom)
            nEnd = string.atol(globalvar.g_argvTryPwdEnd)
            bFound = False
            
            #1234?
            r = "1234"
            if kb.unlockBackupKeybagWithPasscode(r):
                print "Bruteforce successful, backup password : %s" % r
                password = r
                bFound = True
                nFrom = nEnd
            
            #digit 4 ? 6 ?
            nDigits = len(globalvar.g_argvTryPwdEnd)
            for i1 in xrange(nFrom, nEnd + 1):
                r = '%d' % i1
                nFromDigit = len(r)
                for i2 in xrange(nFromDigit, nDigits + 1):
                    #print "Trying %s at %s" % (r, time.strftime('%H:%M:%S', time.localtime(time.time())))
                    if kb.unlockBackupKeybagWithPasscode(r):
                        print "Bruteforce successful, backup password : %s" % r
                        password = r
                        bFound = True
            ...
            print time.strftime('End at %H:%M:%S', time.localtime(time.time()))
            if not bFound:
                print "Password not found! (%s - %s)" % (globalvar.g_argvTryPwdFrom, globalvar.g_argvTryPwdEnd)

        kb.bfPassword = password
