...
class MBDB(object):
    ...
    def extract_backup(self, output_path):
        for record in self.files.values():
            ...
            if record.is_directory():
                try:
                    dirname = record.domain;
                    if dirname.startswith('AppDomain-'):
                        dirname = dirname[10:]
                    fullpath = os.path.join(output_path, dirname, record.path)
                    print(record.domain)
                    # fullpath = unicode(fullpath, 'utf8')
    ...
    def extract_file(self, filename, record, output_path):
        ...
        if dirname.startswith('AppDomain-'):
            dirname = dirname[10:]
        output_path = os.path.join(output_path, dirname, out_file)

        #jsc 20141222       
        #utf8->unicode()
        #output_path = unicode(output_path, 'utf8')
        
        #print("Writing %s" % output_path)
        #print(output_path + " : " + str(type(output_path)))
        
        try:
            f2 = file(output_path, 'wb')
        except(IOError):
            #warn("Path is too long or cannot open: %s" % output_path)
            warn("Path is too long or cannot open: %s" % filename)
            f1.close()
            return
